...
TEST_F(DequeTest, MOVE_CTOR) {
  EXPECT_NO_THROW(deque<int>(mystl::move(di)));
  EXPECT_NO_THROW(deque<float>(mystl::move(df)));
  EXPECT_NO_THROW(deque<double>(mystl::move(dd)));
  EXPECT_NO_THROW(deque<string>(mystl::move(ds)));
  EXPECT_NO_THROW(deque<BAR>(mystl::move(db)));
  EXPECT_NO_THROW(deque<deque<int>>(mystl::move(ddi)));
}
...
TEST_F(DequeTest, COMPARATOR) {
  ...
  deque<string> temp_ds(mystl::move(ds));
