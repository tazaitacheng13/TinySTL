...
TEST_F(QueueTest, MOVE_CTOR) {
  EXPECT_NO_THROW(queue<int>(mystl::move(qi)));
  EXPECT_NO_THROW(queue<float>(mystl::move(qf)));
  EXPECT_NO_THROW(queue<double>(mystl::move(qd)));
  EXPECT_NO_THROW(queue<string>(mystl::move(qs)));
  EXPECT_NO_THROW(queue<BAR>(mystl::move(qb)));
  EXPECT_NO_THROW(queue<queue<int>>(mystl::move(qqi)));
}
