...
namespace mystl {
    ...
    inline ForwardIterator _uninitialized_copy_aux(InputIterator first,
        ...
        ForwardIterator result,
        true_type) {
        return mystl::copy(first, last, result);// in stl_algobase.h
    }
    ...
    inline ForwardIterator uninitialized_copy_copy(InputIterator1 first1,
        ...
        ForwardIterator result) {
        ForwardIterator mid = mystl::uninitialized_copy(first1, last1, result);
        try {
            return mystl::uninitialized_copy(first2, last2, mid);
        }
        catch (std::exception&) {
            mystl::destroy(result, mid);
            throw;
        }
    }
    ...
    inline ForwardIterator uninitialized_fill_copy(ForwardIterator result,
        ...
        InputIterator first,
        InputIterator last) {
        mystl::uninitialized_fill(result, mid, val);
        try {
            return mystl::uninitialized_copy(first, last, mid);
        }
        catch (std::exception&) {
            mystl::destroy(result, mid);
            throw;
    ...
    inline void uninitialized_copy_fill(InputIterator first1, InputIterator last1,
        ...
        ForwardIterator mid2 = mystl::uninitialized_copy(first1, last1, first2);
        try {
            mystl::uninitialized_fill(mid2, last2, val);
        }
        catch (std::exception&) {
            mystl::destroy(first2, mid2);
            throw;
        }
    }
