...
TEST_F(StackTest, MOVE_CTOR) {
  EXPECT_NO_THROW(stack<int>(mystl::move(si)));
  EXPECT_NO_THROW(stack<float>(mystl::move(sf)));
  EXPECT_NO_THROW(stack<double>(mystl::move(sd)));
  EXPECT_NO_THROW(stack<string>(mystl::move(ss)));
  EXPECT_NO_THROW(stack<BAR>(mystl::move(sb)));
  EXPECT_NO_THROW(stack<stack<int>>(mystl::move(ssi)));
}
